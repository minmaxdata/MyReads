...
function filterByShelf(status) {
    return payload.books.filter(function(book) {
        return book.shelf.indexOf(status) > -1;
    })
}

const current = filterByShelf('currentlyReading')
const want = filterByShelf('wantToRead')
const read = filterByShelf('read')
        ...
        return ( <
            div className = "app" > {
                this.state.showSearchPage ? ( <
                    ...
                    ListBookShelf books = { current }
                    /> <
                    /div> <
                    div className = "bookshelf" >
                    <
                    h2 className = "bookshelf-title" > Want to Read < /h2> <
                    ListBookShelf books = { want }
                    /> <
                    /div> <
                    div className = "bookshelf" >
                    <
                    h2 className = "bookshelf-title" > Read < /h2> <
                    ListBookShelf books = { read }
                    /> <
                    /div> <
                    /div> <
                    /div> <
